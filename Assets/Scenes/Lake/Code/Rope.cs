...
public class Rope : MonoBehaviour {
    ...
    private void SimulateRope() {
        for (int i = 0; i < pointCount; i++) {
            ...
            Vector3 velocity =
                point.posNow - point.posOld; //calculates velocity by subtracting old position from current position
    ...
    public void TurnOn() => lineRenderer.enabled = true;

    public void TurnOff() => lineRenderer.enabled = false;

    public void ResetPointCount() => pointCount = maxPointCount;
    ...
    [SerializeField] private readonly float maxSegmentLength = 0.25f; //Maximum distance between two points
    [SerializeField] private readonly float minSegmentLength = 0.01f; //Minimum distance between two points

    public int maxPointCount = 20;
    [HideInInspector] public int pointCount;

    [SerializeField] private readonly float lineWidth = 0.1f; //How thick the line should be
    ...
    private readonly List<RopePoint> ropePoints = new List<RopePoint>();
    ...
    public void ConstrainDistance() {
        for (int i = 0; i < pointCount - 1; i++) {
            ...
            float error = Mathf.Abs(
                dist - ropeSegmentLength); //how much larger or smaller the distance between points is vs. the current segment length
